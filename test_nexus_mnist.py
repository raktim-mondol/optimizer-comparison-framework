"""
NEXUS Optimizer on MNIST with CNN, Early Stopping, and F1 Score

This script trains a CNN model on MNIST using NEXUS optimizer with:
- 100 epochs
- Early stopping
- F1 score tracking
"""
...
def main():
    ...
    print("=" * 80)
    print("NEXUS Optimizer on MNIST with CNN")
    print("=" * 80)
    print("\nConfiguration:")
    print("  - Model: SimpleCNN")
    print("  - Optimizer: NEXUS")
    print("  - Epochs: 100")
    ...
    for epoch in range(num_epochs):
        ...
        print(f'  NEXUS - LR: {lr_stats["mean_adaptive_lr"]:.6f}, '
    ...
    print(f'Best model saved to: nexus_mnist_best.pth')
    print("=" * 80)
...
def plot_training_curves(history, best_epoch):
    ...
    fig.suptitle('NEXUS Optimizer Training Progress on MNIST', fontsize=16, fontweight='bold')
    ...
    axes[1, 1].plot(epochs, [1.0] * len(epochs), 'k--', alpha=0.3, label='Initial LR')
    axes[1, 1].set_xlabel('Epoch', fontsize=12)
    axes[1, 1].set_ylabel('Adaptive LR', fontsize=12)
    axes[1, 1].set_title('Adaptive Learning Rate', fontsize=14, fontweight='bold')
    axes[1, 1].legend(fontsize=10)
    axes[1, 1].grid(True, alpha=0.3)
    
    # Curvature over time
    axes[1, 2].plot(epochs, [1.0] * len(epochs), 'k--', alpha=0.3, label='Initial Curvature')
    axes[1, 2].set_xlabel('Epoch', fontsize=12)
    axes[1, 2].set_ylabel('Curvature', fontsize=12)
    axes[1, 2].set_title('Curvature Estimates', fontsize=14, fontweight='bold')
    axes[1, 2].legend(fontsize=10)
    axes[1, 2].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('nexus_mnist_training_curves.png', dpi=150, bbox_inches='tight')
    print(f'\nTraining curves saved to: nexus_mnist_training_curves.png')
    plt.close()
