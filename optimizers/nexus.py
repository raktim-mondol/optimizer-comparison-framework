"""
NEXUS: Neural EXploration with Unified Scaling

A novel optimizer that combines multiple advanced techniques for faster convergence
and better performance than existing methods.

Key Innovations:
1. Meta-Learning Adaptive Learning Rates (MLALR): Lightweight meta-learning approach
   to adapt learning rates per parameter based on historical performance
2. Multi-Scale Momentum (MSM): Multiple momentum buffers with different time scales
   for capturing both short-term and long-term gradient trends
3. Gradient Direction Consistency (GDC): Track gradient direction consistency and
   adjust momentum accordingly for more stable updates
4. Layer-wise Adaptation (LWA): Different adaptation strategies for different layers
   based on their depth and importance
5. Adaptive Step Size with Lookahead (ASSL): Combine current step with lookahead
   mechanism for better convergence
6. Dynamic Weight Decay (DWD): Adapt weight decay based on parameter importance
...
- Combines ideas from meta-learning, adaptive optimization, and trust-region methods
- Uses a unified scaling mechanism that balances exploration and exploitation
...
Args:
    ...
    meta_lr: Meta-learning rate for adaptive learning rates (default: 1e-4)
...
class NEXUS(BaseOptimizer):
    """
    Neural EXploration with Unified Scaling (NEXUS) Optimizer.
    
    A novel optimizer combining multiple advanced techniques for faster convergence
    and better performance than existing methods.
    
    Key Features:
    1. Meta-Learning Adaptive Learning Rates (MLALR)
    2. Multi-Scale Momentum (MSM) with 3 momentum buffers
    3. Gradient Direction Consistency (GDC) tracking
    4. Layer-wise Adaptation (LWA) based on depth
    5. Adaptive Step Size with Lookahead (ASSL)
    6. Dynamic Weight Decay (DWD)
    7. Curvature-Aware Scaling (CAS)
    8. Gradient Noise Injection (GNI)
    ...
    def __init__(self, params, lr: float = 1e-3, betas: Tuple[float, float, float] = (0.9, 0.99, 0.999),
                 eps: float = 1e-8, weight_decay: float = 0, meta_lr: float = 1e-4,
                 momentum_scales: int = 3, direction_consistency_alpha: float = 0.1,
                 layer_adaptation: bool = True, lookahead_steps: int = 5,
                 lookahead_alpha: float = 0.5, noise_scale: float = 0.0,
                 noise_decay: float = 0.99, curvature_window: int = 10,
                 adaptive_weight_decay: bool = True, weight_decay_alpha: float = 0.01,
                 max_grad_norm: float = 1.0, amsgrad: bool = False,
        ...
        defaults = dict(
            ...
            meta_lr=meta_lr, momentum_scales=momentum_scales,
            direction_consistency_alpha=direction_consistency_alpha,
            ...
            adaptive_weight_decay=adaptive_weight_decay,
            weight_decay_alpha=weight_decay_alpha, max_grad_norm=max_grad_norm,
            amsgrad=amsgrad, state_precision=state_precision
        )
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    # Adaptive learning rate buffer
                    state['adaptive_lr'] = torch.ones_like(p, dtype=dtype,
                                                          memory_format=torch.preserve_format)
                    ...
                    # Initialize slow parameters for lookahead
    ...
    def _compute_layer_factors(self, params: List[torch.Tensor], layer_adaptation: bool) -> List[float]:
        ...
        for i, p in enumerate(params):
            ...
            depth_factor = 1.0 + 0.5 * (1.0 - i / len(params))
    ...
    def _nexus_update(self,
                      ...
                      amsgrad: bool):
        """
        Perform the NEXUS parameter update.
        """
        for i, param in enumerate(params):
            ...
            if weight_decay != 0:
                if adaptive_weight_decay:
                    ...
                    grad = grad.add(param * adaptive_wd)
                else:
            ...
            if step > 1:
                ...
                direction_consistency.mul_(1 - direction_consistency_alpha).add_(
                    torch.clamp(similarity, min=0.0), alpha=direction_consistency_alpha
                )
            ...
            # Update adaptive learning rate based on gradient statistics
    ...
    def get_adaptive_lr_stats(self) -> Dict[str, float]:
        """
        Get statistics about adaptive learning rates.
        
        Returns:
            Dictionary containing adaptive learning rate statistics
        """
        stats = {
            'mean_adaptive_lr': 0.0,
            'max_adaptive_lr': float('-inf'),
            'min_adaptive_lr': float('inf'),
            'std_adaptive_lr': 0.0,
        }
            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    if 'adaptive_lr' in state:
                        adaptive_lr_flat = state['adaptive_lr'].flatten()
                        lr_values.extend(adaptive_lr_flat.tolist())
        
        if lr_values:
            import numpy as np
            stats['mean_adaptive_lr'] = np.mean(lr_values)
            stats['max_adaptive_lr'] = np.max(lr_values)
            stats['min_adaptive_lr'] = np.min(lr_values)
            stats['std_adaptive_lr'] = np.std(lr_values)
    ...
    def get_momentum_stats(self) -> Dict[str, List[float]]:
        ...
        """
        stats = {
            'scale_0': {'mean': 0.0, 'std': 0.0},
            'scale_1': {'mean': 0.0, 'std': 0.0},
            'scale_2': {'mean': 0.0, 'std': 0.0},
        }
        ...
        for scale_idx in range(3):
            ...
            if momentum_values:
                import numpy as np
                stats[f'scale_{scale_idx}']['mean'] = np.mean(momentum_values)
                stats[f'scale_{scale_idx}']['std'] = np.std(momentum_values)
